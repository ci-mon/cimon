...
public sealed class DeployTask : FrostingTask<BuildContext>
{
	public override void Run(BuildContext context) {
		...
		context.StartProcess("robocopy", new ProcessSettings {
			Arguments = new ProcessArgumentBuilder().AppendQuoted(context.PublishDir.FullPath).AppendQuoted(deployPath)
				.Append("/MIR").Append("/R:15").Append("/W:5").Append("/XD").AppendQuoted("nativeApps")
				.AppendQuoted("db").AppendQuoted("logs").Append("/XF").AppendQuoted(webConfig).Append("/XF")
				.AppendQuoted("appsettings.Production.json")
		});
